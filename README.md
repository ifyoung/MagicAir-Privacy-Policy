...
1. 适用范围
a) 在您注册MagicAir帐号时，您根据MagicAir要求提供的个人注册信息；
b) 在您使用MagicAir网络服务，或访问MagicAir平台网页时，MagicAir自动接收并记录的您的浏览器和计算机上的信息，包括但不限于您的IP地址、浏览器的类型、使用的语言、访问日期和时间、软硬件特征信息及您需求的网页记录等数据；
c) MagicAir通过合法途径从商业伙伴处取得的用户个人数据。
您了解并同意，以下信息不适用本隐私权政策：
a) 您在使用MagicAir平台提供的搜索服务时输入的关键字信息；
b) MagicAir收集到的您在MagicAir发布的有关信息数据，包括但不限于参与活动、成交信息及评价详情；
c) 违反法律规定或违反MagicAir规则行为及MagicAir已对您采取的措施。
